...
const CustomerPointsModal = ({ onClose, isOpen }) => {
  const { customerTotalPoints, customerPhoneNumber, setCustomer10PercentOff } = useGlobalState();
  const [modalMessage, setModalMessage] = useState('');

  useEffect(() => {
    if (customerTotalPoints >= 1000) {
      setModalMessage(`You have ${customerTotalPoints} points! Would you like to use 1000 points for a 10% discount?`);
    } else {
      const pointsNeeded = 1000 - customerTotalPoints;
      setModalMessage(`You are ${pointsNeeded} points away from earning 1000 points for a 10% discount.`);
    }
  }, [customerTotalPoints]);
  ...
  return (
    <Modal open={isOpen} onClose={onClose} aria-labelledby="customer-points-modal" aria-describedby="customer-points-description">
      <div className={styles.modalContent}>
        ...
        {customerTotalPoints >= 1000 ? (
          ...
          <div>
            <button className={`${styles.button} ${styles.redButton}`} onClick={onClose}>
              Close
            </button>
          </div>
        )}
